...
app = modal.App("music-saas")

image = (              #these all are cached in the image, first it will always take time
...
music_gen_secrets = modal.Secret.from_name("music-saas")  # Music generation secrets
...
@app.cls(
    image=image,
    gpu="L40S",
    volumes={"/models": model_volume, "/.cache/huggingface": hf_volume},    #when ever we attch this volume it will be cached and will persist across invocations
    secrets=[music_gen_secrets],
    scaledown_window=15                  #scale down after 15s of inactivity , still we have to pay for the 15 seconds moer as itsis good in production
)
...
class MusicGenServer:
    ...
    def generate(self) -> GenerateMusicResponse:
        ...
        self.music_model(     #call the music generation model in the modal server, in encoded form it is returned form the cloud
